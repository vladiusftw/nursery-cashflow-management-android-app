...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void insertStaff(Staff staff) {
        ...
        values.put("pic", FunctionsHelper.getBytes(staff.getPic()));
    ...
    public ArrayList<Staff> getStaff() { // display all of the staff members
        ...
        while (cursor.moveToNext()){
            staffList.add(new Staff(
                    ...
                    FunctionsHelper.getImage(cursor.getBlob(picIndex))));
        }
        return staffList;
    }
    ...
    public ArrayList<Expense> getNurseryExpensesByMonth( int month ) {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + EXPENSE_TABLE_NAME + " WHERE kidId = 0", null);
    ...
    public ArrayList<Expense> getStatementsByMonth(int month){
        ArrayList<Expense> expenses = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + EXPENSE_TABLE_NAME + " WHERE kidId != 0", null); // where foreign key is null ont forget to add!!!!!
        int detailIndex = cursor.getColumnIndex("detail");
        int amountIndex = cursor.getColumnIndex("amount");
        int dateIndex = cursor.getColumnIndex("date");
        while (cursor.moveToNext()) {
